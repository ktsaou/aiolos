...
//! Recognized directive: `input=<peer>` (relay that peer's components into this module's `apply`).
...
impl RegistryEntry {
    ...
    /// components by `module:id`, so a `:` in the name would make a source prefix match the wrong
...
mod tests {
    ...
    fn module_with_input() {
        let e = parse_module_line("rome2d-fans  input=nvidia");
        assert_eq!(e.module_name, "rome2d-fans");
        assert_eq!(e.inputs, vec!["nvidia".to_string()]);
    }

    #[test]
    fn multiple_inputs_repeated_or_comma_dedup_and_ordered() {
        let repeated = parse_module_line("rome2d-fans input=nvidia input=nvme");
        assert_eq!(
            repeated.inputs,
            vec!["nvidia".to_string(), "nvme".to_string()]
        );

        let comma = parse_module_line("rome2d-fans input=nvidia,nvme");
        assert_eq!(comma.inputs, vec!["nvidia".to_string(), "nvme".to_string()]);

        // Duplicates are dropped; first-seen order preserved.
        let dup = parse_module_line("rome2d-fans input=nvidia input=nvidia,nvme");
