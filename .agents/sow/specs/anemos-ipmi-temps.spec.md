...
controls NO device. Conforms to `aiolos-protocol.spec.md`. Its components are routed
(`input=ipmi-temps`) into a fan controller (`rome2d-fans`) so a hot board/DIMM/NIC raises the board
fans.
...
and the `rome2d-fans` module's own `k10temp` CPU reads — the board-fan driving max already folds in
**all** routed temperature publishers source-agnostically.

## Hardware facts
- BMC: ASPEED AST2500 (MegaRAC) on the ROME2D16-2T; inband interface `/dev/ipmi0`. Same BMC the
  `rome2d-fans` module drives.
- Analog temperature sensors are read with **standard** IPMI (netfn `0x04`): `Get Sensor Reading`
  ...
  conversion — the identical mechanism the `rome2d-fans` module uses for fan-tach RPM (SOW-0005), here
  applied to temperatures.
- Sensor numbers (board-specific, hardcoded — like `rome2d-fans` `FAN_SENSORS`):
...
## run <board>
- Read every hardcoded sensor and report one `bmc` component with one `temperature` publisher per
  sensor whose reading is
  **available**; unavailable/"ns" sensors are skipped (never reported as 0/garbage):
  ```json
  {"status":"ok","components":[{
    "id":"bmc","label":"ROME2D16-2T BMC temps","class":"board",
    "publishers":[
      {"id":"temp.mb1","label":"MB1","kind":"temperature","value":26,"unit":"C"},
      {"id":"temp.lan","label":"LAN","kind":"temperature","value":47,"unit":"C"}]}]}
  ```
...
- All reads use a **short timeout** (100 ms) so a slow/unresponsive BMC degrades a sensor to "absent"
  rather than blowing the apply deadline (same policy as the `rome2d-fans` observability reads).
...
folding the temps into the board-fan module that already holds an IPMI handle) preserves the
...
in the module (`anemoi/ipmi-temps/src/sensors.rs`), mirroring `anemoi/rome2d-fans/src/board.rs`. A
...
## Modes
`detect` · `info [id]` / `collect [id]` · `run <board>` · `restore` (one-shot: **no-op** — a sensor controls nothing — exits 0;
...
- `run` reports one `temperature` publisher per **available** BMC sensor within timeout, with the labels
...
- Values match `ipmitool sdr type Temperature` (on-hardware validation).
- Routed `input=ipmi-temps` into `rome2d-fans`: a warm DIMM/NIC raises the board fans (the
  board-fan driving max already folds in all routed temps source-agnostically).
